...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String identifier) throws UsernameNotFoundException {
        ...
        var doctorOpt = doctorRepository.findByEmailOrPhone(identifier, identifier);
        if (doctorOpt.isPresent()) {
            var doctor = doctorOpt.get();
            return org.springframework.security.core.userdetails.User.builder()
                .username(doctor.getEmail()!= null ? doctor.getEmail() : doctor.getPhone())
                .password(doctor.getPassword()) // stored hashed password
                .roles(doctor.getRole().name())
                .build();
        } else {
            System.out.println("Doctor not found with email or phone: " + identifier);
        }
